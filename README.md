...
> Self-hosted, multi-broadcaster live video over your local network. Mobile cameras stream to any viewer with sub-100ms latency, plus tap-to-talk, remote alarm, server-side recording, AI object detection, and remote camera control — all over WebRTC peer-to-peer with zero cloud services.
...
- 🎯 **Run AI object detection** in the browser and ring an alarm when a target object appears
...
### Broadcasting
- 📱 **Mobile-first interface** — open in a phone browser, camera preview is instant
...
- 🎯 **AI object detection** — pick from 80 classes, ring local alarm when detected
...
5. **Object Detection** — AI-based watching
6. **Connection** — live stats (FPS, bitrate, P2P state, resolution)

**Player bar:**
- 🔇/🔊 mute toggle, volume slider, 📸 snapshot, ⊞ Picture-in-Picture, ⛶ fullscreen
...
In the **Object Detection** panel, tap **⚡ Enable AI Detection** (one-time, ~5 MB model download from CDN).

Once loaded:
1. Pick a target object from the grouped dropdown (80 classes available)
2. Tap **👁️ Start Watching**
3. The model scans the video every 1.5 seconds
4. When a match appears:
   - 🔴 Red bounding box drawn over the object on the video
   - 🚨 Local siren rings on **your** device (different tone from the remote alarm)
   - Status panel shows confidence percentage
   - Alarm auto-stops 3 seconds after the object leaves the frame

**80 detectable objects** (COCO-80), grouped:
...
> **Want to detect something else?** See [Extending the AI Detection](#extending-the-ai-detection) below.

**Performance:** ~100-300 ms per inference on a modern device, runs at ~0.7 FPS detection rate (configurable in code). All inference is client-side — no data sent to the server.
...
The default model is **COCO-SSD** (80 classes). To detect other objects:

### Option 1: Teachable Machine *(easiest, ~5 minutes)*
[teachablemachine.withgoogle.com](https://teachablemachine.withgoogle.com) — train a classification or detection model in your browser, export to TF.js, and replace the model load:

```javascript
// In view.html, find the comment "// ─── Object Detection ───"
// Replace cocoSsd.load() with:
cocoModel = await tf.loadLayersModel('https://your-storage/model.json');
```

### Option 2: Roboflow Universe *(pre-trained, free)*
[universe.roboflow.com](https://universe.roboflow.com) — find a model trained on your specific objects (fire, helmets, masks, packages, weapons, etc.) and export to TF.js format.
...
```
Then load with `tf.loadGraphModel()` and adapt the inference call (YOLOv8 outputs are different from COCO-SSD).

### Option 4: Server-side Python *(heaviest, most flexible)*
Run a small FastAPI service with full PyTorch/Ultralytics. The viewer captures frames and POSTs them to the Python service, which returns detections. Drop-in replacement for the `cocoModel.detect(video)` call.
...
- 🤖 Multiple custom detection models with model picker UI
